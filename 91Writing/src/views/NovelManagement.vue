  ...
  try {
    ...
    if (response.ok) {
      ...
      novels.value = data.map(novel => ({
        ...novel,
        createdAt: new Date(novel.created_at || novel.createdAt),
        updatedAt: new Date(novel.updated_at || novel.updatedAt),
        chapterList: [],
        writingRecords: []
      }))
    } else {
      console.error('加载小说列表失败:', response.status)
      novels.value = []
    }
  } catch (error) {
...
// 保存小说数据（兼容性保留，实际通过API操作）
const saveNovels = () => {
  // 不再使用localStorage，数据通过API持久化
  console.log('saveNovels called - data is managed by backend API')
}
...
const deleteNovel = async (novel) => {
  try {
    ...
    const response = await fetch(`/api/novels/${novel.id}/delete`, {
      method: 'POST'
    })
...
const createNovel = async () => {
  try {
    ...
    const payload = {
      ...
      cover_image: createForm.value.cover,
      status: 'ongoing'
    }
