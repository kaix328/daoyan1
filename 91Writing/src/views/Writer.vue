<template>
  <div class="writer-container">
    ...
    <div class="title-bar">
      ...
      <div class="title-right">
        <el-dropdown @command="handleMoreCommand">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="rolling-summary">
                <el-icon><MagicStick /></el-icon>
                故事摘要 (Rolling Summary)
              </el-dropdown-item>
              <el-dropdown-item command="export-full">
    ...
    <!-- 滚动摘要对话框 -->
    <el-dialog v-model="showRollingSummaryDialog" title="故事滚动摘要 (Rolling Summary)" width="700px">
      <div class="rolling-summary-container">
        <div class="summary-tip">
          <el-alert
            title="滚动摘要是AI对整部小说剧情的长期记忆，会自动整合最新的章节发展。您可以手动编辑修正，以确保AI理解正确。"
            type="info"
            show-icon
            :closable="false"
          />
        </div>
        <div class="summary-content" style="margin-top: 16px;">
          <el-input
            v-model="rollingSummary"
            type="textarea"
            :rows="15"
            placeholder="暂无摘要，点击底部的'AI生成摘要'按钮开始生成..."
          />
        </div>
      </div>
      <template #footer>
        <div class="dialog-footer" style="display: flex; justify-content: space-between; align-items: center;">
          <div class="left-actions">
            <el-button 
              type="primary" 
              plain 
              @click="generateRollingSummary(false)" 
              :loading="isGeneratingRollingSummary"
            >
              <el-icon><MagicStick /></el-icon>
              AI生成/更新摘要
            </el-button>
          ...
          <div class="right-actions">
            <el-button @click="showRollingSummaryDialog = false">取消</el-button>
            <el-button type="primary" @click="saveRollingSummary">保存摘要</el-button>
          </div>
        </div>
      </template>
    </el-dialog>
...
import { 
  ArrowLeft, DocumentAdd, Plus, Edit, Delete, Document, MoreFilled, ArrowDown, Star, Tools, ArrowRight, Right, Check, InfoFilled, MagicStick, Close, CopyDocument, Download, Setting
...
// 滚动摘要相关数据
const showRollingSummaryDialog = ref(false)
const rollingSummary = ref('')
const isGeneratingRollingSummary = ref(false)
const rollingSummaryPromptVariables = ref({})
...
const autoUpdateRollingSummary = async () => {
  // 每完成3章自动更新一次摘要
  if (chapters.value.length % 3 === 0 && chapters.value.length > 0) {
    try {
      await generateRollingSummary(true) // true 表示自动模式，不显示弹窗
    } catch (e) {
      console.error('自动更新摘要失败:', e)
    }
  }
}

// 监听章节数量变化，触发自动摘要更新
watch(() => chapters.value.length, (newVal, oldVal) => {
  if (newVal > oldVal) {
    autoUpdateRollingSummary()
  }
})

// 生成滚动摘要
const generateRollingSummary = async (isAuto = false) => {
  if (!checkApiAndBalance()) return
  
  isGeneratingRollingSummary.value = true
  if (!isAuto) showRollingSummaryDialog.value = true
  
  try {
    // 获取需要摘要的章节（假设为最近未摘要的3章，或者全量重新生成）
    // 这里采用全量+增量策略：基于现有摘要 + 新增章节生成新的摘要
    
    const context = buildGenerationContext()
    const currentSummary = rollingSummary.value || '暂无摘要'
    const allChaptersContent = chapters.value.map((c, i) => `第${i+1}章：${c.title}\n${c.description || ''}`).join('\n')
    
    const prompt = `=== 小说基本信息 ===
标题：${currentNovel.value?.title}
类型：${currentNovel.value?.genre}
...
请输出新的摘要内容：`
    ...
    const aiResponse = await apiService.generateTextStream(prompt, {
      ...
      rollingSummary.value = fullContent
    })
    
    if (isAuto) {
      ElMessage.success('故事摘要已自动更新')
    } else {
    ...
    if (!isAuto) ElMessage.error('生成摘要失败: ' + error.message)
  } finally {
    isGeneratingRollingSummary.value = false
  }
}

// 手动打开摘要对话框
const openRollingSummaryDialog = () => {
  showRollingSummaryDialog.value = true
}

// 保存摘要修改
const saveRollingSummary = () => {
  saveNovelData()
  showRollingSummaryDialog.value = false
...
const handleMoreCommand = (command) => {
  switch (command) {
    case 'settings':
      ElMessage.info('小说设置功能开发中...')
      break
    case 'export-full':
      ElMessage.info('导出全文功能开发中...') 
      // TODO: Implement export full text
      break
    case 'rolling-summary':
      openRollingSummaryDialog()
      break
  }
}
...
请确保生成的章节符合小说的整体风格、类型和世界观设定。`
    ...
    newChapters.forEach((chapterData, index) => {
      const newChapter = {
        novel_id: currentNovel.value.id, // Ensure link to novel
        title: chapterData.title || `AI生成章节 ${chapters.value.length + index + 1}`,
        description: chapterData.description || chapterData.outline || '暂无描述',
        content: '',
        wordCount: 0,
        status: 'draft',
        order_index: chapters.value.length + index
      }
      // Note: We don't push to chapters.value immediately if we want to save to DB first
      // But for UI responsiveness, we might. However, best practice is to save then reload or push with returned ID.
      // For now, let's just push and let the save handle it.
      // actually, db_save_chapter returns ID. We should probably save them one by one or batch.
      // Simplified: Just push to local view, let user save individual chapters or auto-save loop handle it.
      chapters.value.push(newChapter)
    })
    
    ElMessage.success(`成功生成${newChapters.length}个章节大纲`)
    saveNovelData() // Calls API
  } catch (error) {
    console.error('AI生成章节失败:', error)
    ElMessage.error(`章节生成失败: ${error.message}`)
  } finally {
...
const buildGenerationContext = () => {
  ...
  return {
    ...
    rollingSummary: rollingSummary.value, // 新增：滚动摘要
    // 新增：更详细的上下文信息
...
const saveNovelData = async () => {
  ...
  currentNovel.value.wordCount = totalWordCount
  ...
  await novelStore.saveNovelData(extraData, rollingSummary.value)
}
