...
export const useNovelStore = defineStore('novel', () => {
  ...
  const initializeApiConfig = () => {
    try {
      ...
      // 确保官方配置有默认Key
      if (!officialApiConfig.value.apiKey) {
        officialApiConfig.value.apiKey = 'internal'
      }
      ...
      // 如果是官方配置，始终认为已配置（使用代理）
      isApiConfigured.value = currentConfigType.value === 'official' || !!currentConfig.apiKey
  ...
  const switchConfigType = (type) => {
    ...
    const currentConfig = getCurrentApiConfig()
    apiService.updateConfig(currentConfig)
    isApiConfigured.value = type === 'official' || !!currentConfig.apiKey
  }
  ...
  // --- Data Persistence using Backend API ---
  ...
  // Save Novel Metadata
  const saveNovelData = async (extraData = null, rollingSummary = null) => {
    ...
    try {
      const payload = {
        ...
        description: currentNovel.value.description,
        genre: currentNovel.value.genre,
        cover_image: currentNovel.value.cover_image,
        status: currentNovel.value.status,
        updated_at: new Date(),
        extra_data: extraData,
        rolling_summary: rollingSummary
      }

      const response = await fetch('/api/novels/save', {
        method: 'POST',
  ...
  const saveCurrentChapter = async () => {
    if (!selectedChapter.value || !currentNovel.value?.id) return
    ...
    try {
      const payload = {
        ...
        word_count: selectedChapter.value.content?.length || 0,
        status: selectedChapter.value.status,
        order_index: selectedChapter.value.order_index || 0
      }
      ...
      if (response.ok) {
        ...
        if (!selectedChapter.value.id) {
          selectedChapter.value.id = data.id
        }
  ...
  const deleteNovel = async (novelId) => {
    try {
      await fetch('/api/novels/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: novelId })
      })
    } catch (error) {
